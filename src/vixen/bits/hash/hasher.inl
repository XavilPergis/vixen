...
constexpr void FxHasher::write_bytes(void const *data, usize len) {
    void const *aligned = util::alignPointerUp(data, alignof(u64));
    void const *cur = data;
    void const *end = util::offsetRaw(data, len);

    while (cur < aligned) {
        detail::fx_hasher_add_hash(current, (u64)util::readRawAs<u8>(cur));
        cur = util::offsetRaw(cur, 1);
    }

    while (util::offsetRaw(cur, sizeof(u64)) < end) {
        detail::fx_hasher_add_hash(current, util::readRawAs<u64>(cur));
