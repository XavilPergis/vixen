...
#define _XEN_UTF8_CODEPOINT_VALID(codepoint)                               \
    XEN_ENGINE_DEBUG_ASSERT(::xen::utf8::is_valid_for_encoding(codepoint), \
...
inline string::string(vector<char> &&data) : data(MOVE(data)) {
    // XEN_ENGINE_ASSERT(utf8::is_valid(data), "String data was not valid UTF-8.")
}
...
inline void string::push(u32 codepoint) {
    _XEN_UTF8_CODEPOINT_VALID(codepoint)
...
inline void null_terminate(vector<char> *data) {
    ...
    XEN_ENGINE_ASSERT(invalid_nuls == 0,
...
inline string_slice string_slice::operator[](range r) const {
    XEN_ENGINE_DEBUG_ASSERT(utf8::is_char_boundary(raw[r.start]),
...
inline string_slice string_slice::operator[](range_from range) const {
    XEN_ENGINE_DEBUG_ASSERT(utf8::is_char_boundary(raw[range.start]),
        "Tried to slice string, but start index {} was not on a char boundary.",
        range.start)
