...
inline void vector<T>::truncate(usize len) {
    XEN_ENGINE_DEBUG_ASSERT(len <= length,
...
inline T vector<T>::remove(usize idx) {
    XEN_ENGINE_DEBUG_ASSERT(length > idx,
...
inline void vector<T>::swap(usize a, usize b) {
    XEN_ENGINE_DEBUG_ASSERT((length > a) && (length > b),
...
inline void vector<T>::set_capacity(usize cap) {
    XEN_ENGINE_ASSERT(alloc != nullptr, "Tried to grow a defaulted Vec.");
