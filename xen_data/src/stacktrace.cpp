...
#define FAIL(...)                         \
    {                                     \
        XEN_ENGINE_CRITICAL(__VA_ARGS__); \
        ::std::abort();                   \
    }
...
address_info const &translation_cache::get_info(void *addr) const {
    XEN_ENGINE_ASSERT(translated.key_exists(addr), "Address {} was not translated yet.", addr)
...
void print_stack_trace_capture(slice<const address_info> info) {
    XEN_ENGINE_INFO("Stack was {} calls deep:", info.len)
    for (usize i = 0; i < info.len; ++i) {
        XEN_ENGINE_INFO("\t\x1b[36m{: >2}\x1b[0m :: \x1b[1m\x1b[32m{}\x1b[0m", i + 1, info[i].name)
        XEN_ENGINE_INFO("\t   :: in {}", info[i].location)
        XEN_ENGINE_INFO("\t   :: symbol {}", info[i].raw_symbol)
        XEN_ENGINE_INFO("\t   ==")
    }
}
