...
struct map_iterator {
    ...
    struct reified {
        ...
        bool has_next() { return downstream.has_next(); }

        output next() { return mapper(downstream.next()); }
    };
};
...
struct iter_to_std : public std::iterator<std::input_iterator_tag, typename Iter::output> {
    explicit iter_to_std(Iter &&iter) : iter(mv(iter)) { load_next(); }
    ...
    iter_to_std begin() const { return mv(*this); }

    iter_to_std end() const { return iter_to_std(); }
...
inline iterator_pipeline<Bs..., As...> operator|(iterator_pipeline<As...> &&tail,
    iterator_pipeline<Bs...> &&head) {
