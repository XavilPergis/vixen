...
struct result_impl {
    ...
    result_impl(detail::ok<U> &&val) : ok_flag(true) {
        ok_storage.constructInPlace(std::forward<U>(val.value));
    }
    template <typename U>
    result_impl(detail::err<U> &&val) : ok_flag(false) {
        err_storage.constructInPlace(std::forward<U>(val.value));
    }
    ...
    result_impl(result_impl<T, E> &&other) {
        if (other.is_ok()) {
            ok_storage.constructInPlace(mv(other.get_ok()));
        } else {
            err_storage.constructInPlace(mv(other.get_err()));
        }
    }
    ...
    template <typename U>
    void set_ok(U &&val) { return ok_storage.constructInPlace(std::forward<U>(val)); }
    T &get_ok() { return ok_storage.get(); }
    const T &get_ok() const { return ok_storage.get(); }

    template <typename U>
    void set_err(U &&val) { return err_storage.constructInPlace(std::forward<U>(val)); }
...
struct result {
    ...
    bool is_ok() const { return impl.is_ok(); }
    bool is_err() const { return !impl.is_ok(); }
